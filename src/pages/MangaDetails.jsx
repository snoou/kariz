  ...
  if (params.id === 'hazhir-sensei') {
    return {
      manga: {
        ...
        synopsis:
          'در دورانی که باگ‌ها گرند لاین را تسخیر کرده بودند و کدها در تاریکی کامپایل نمی‌شدند، استادی از دل کوه‌های فوجی با کیبوردی از جنس نور طلوع کرد... گفته می‌شود او می‌تواند ری‌اکت را فقط با نگاه کردن به مانیتور دیباگ کند.',
    ...
    if (manga.isEasterEgg) {
      ...
      setTimeout(() => {
        setTranslatedSynopsis(
          'نیاز به ترجمه نیست! کدهای استاد هژیر به تمام زبان‌های برنامه‌نویسی و انسانیِ کیهان قابل فهم است.'
        );
          ...
          messages: [
            {
              role: 'system',
              content:
                'تو یک مترجم حرفه‌ای مانگا هستی. لحن ترجمه‌ات باید جذاب و روان باشد.',
            },
            {
              role: 'user',
              content: `این داستان را به فارسی روان ترجمه کن:
...
${manga.synopsis}`,
      ...
      setTranslatedSynopsis(content);
    } catch (error) {
      console.error('خطا در ترجمه:', error);

      setTranslatedSynopsis(
        'خطا در ارتباط با سرور هوش مصنوعی برای ترجمه.'
      );
    } finally {
      setIsTranslating(false);
    }
  ...
  const handleDeepAnalysis = async () => {
      ...
      setTimeout(() => {
        setSibylData({
          character_psychology:
            'سطح انرژی روانی این کاراکتر غیرقابل اندازه‌گیری است. مواجهه با کدهای تمیز او باعث کاتارسیس برنامه‌نویسان و وحشت باگ‌ها می‌شود.',

          powers_and_inspirations: [
            {
              power: 'کیبورد نوری (The Radiant Keyboard)',
              inspiration:
                'الهام گرفته از افسانه‌های باستانیِ توسعه‌دهندگان سینیور که بدون موس و فقط با شورت‌کات‌ها کد می‌زنند.',
            },
            {
              power: 'چشمِ کامپایلر (Compiler\'s Eye)',
              inspiration:
                'توانایی دیدن ارورها قبل از ذخیره کردن فایل؛ الهام گرفته شده از شارینگان در خاندان اوچیها.',
            },
          ],
        });
    ...
    try {
      const genres =
        manga.genres?.map((g) => g.name).join('، ') || 'نامشخص';

      const themes =
        manga.themes?.map((t) => t.name).join('، ') || 'نامشخص';
...
شما باید خروجی را دقیقاً در قالب یک آبجکت JSON معتبر و بدون تگ مارک‌داون برگردانی.

ساختار خروجی باید دقیقاً این باشد:
...
نکته:
اگر داستان قدرت ماورایی ندارد، مهارت‌های برجسته شخصیت‌ها را بنویس و بگو از چه چیزی در دنیای واقعی الهام گرفته‌اند.
...
خلاصه: ${manga.synopsis}`,
      ...
      if (
        !parsedData.character_psychology ||
        !Array.isArray(parsedData.powers_and_inspirations)
      ) {
        throw new Error('ساختار JSON دریافت شده نامعتبر است');
      }
      ...
      setAnalysisError(
        'ارتباط با هسته تحلیلی قطع شد یا فرمت دیتا نامعتبر بود. لطفاً دوباره تلاش کنید.'
      );
    } finally {
      setIsAnalyzing(false);
    }
      ...
      <div className="comic-panel p-6 md:p-10 mb-12 flex flex-col md:flex-row gap-8 bg-white relative">
        ...
        <div className="w-full md:w-2/3 flex flex-col">
          ...
          <div className="flex flex-wrap gap-4 mb-6 font-comic text-lg">
            <span className="bg-blue-400 border-2 border-black px-3 py-1 shadow-[2px_2px_0_0_#000]">
              امتیاز: {manga.score || 'نامشخص'}
            </span>

            <span className="bg-green-400 border-2 border-black px-3 py-1 shadow-[2px_2px_0_0_#000]">
              وضعیت: {manga.status}
            </span>
          </div>

          <div className="bg-gray-100 p-5 border-3 border-black relative mb-6">
            <div className="absolute -top-4 left-4 bg-black text-white font-comic px-3 py-1 text-sm shadow-[2px_2px_0_0_#cbd5e1]">
              خلاصه داستان
            </div>

            <p
              className="text-gray-800 leading-loose text-justify font-medium mt-2"
              dir="ltr"
            >
          ...
          {translatedSynopsis && (
            <div className="bg-purple-100 p-5 border-3 border-black border-dashed relative">
              <div className="absolute -top-4 right-4 bg-purple-600 text-white font-comic px-3 py-1 text-sm shadow-[2px_2px_0_0_#000]">
                ترجمه جادویی
              </div>

              <p
                className="text-gray-900 leading-loose text-justify font-bold mt-2"
                dir="rtl"
              >
                {translatedSynopsis}
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            ...
            <div className="comic-panel p-6 bg-indigo-100 relative transform rotate-1">
              ...
              <ul className="mt-4 space-y-4">
                {sibylData.powers_and_inspirations.map(
                  (item, index) => (
                    <li
                      key={index}
                      className="bg-white border-3 border-black p-4 shadow-[4px_4px_0_0_#000]"
                    >
          ...
          <div className="mt-12 comic-panel p-8 bg-zinc-900 border-4 border-yellow-500 relative transform -rotate-1 animate-fade-in transition-all">
            ...
            <div className="mt-6 flex flex-col md:flex-row items-center gap-6">
              <div className="w-32 h-32 bg-yellow-400 border-4 border-black flex items-center justify-center rounded-full shadow-[4px_4px_0_0_#fff] shrink-0 transform rotate-6">
                <span className="font-comic text-6xl font-bold">
                  H
                </span>
              </div>
              ...
              <div className="text-white space-y-4">
                ...
                <p
                  className="leading-relaxed text-justify text-lg font-bold text-gray-200"
                  dir="rtl"
                >
                  «ای کاوشگرِ کدهای پنهان... تو به هسته‌ی مرکزی سیبیل رسیدی.
                  آنجا که منطقِ صفر و یک با هنر درمی‌آمیزد و باگ‌ها همچون
                  دیوهای سپید به زانو درمی‌آیند.
                  عبور تو از دروازه‌های این ماژول، نشانِ یک شینوبیِ راستین
                  در مسیرِ توسعه است.
